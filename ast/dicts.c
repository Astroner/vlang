static char* NodeKind[] = {
    "AST_KIND_BLANK",
    "AST_KIND_VARIABLE_DECLARATION",
    "AST_KIND_FUNCTION_CALL",
    "AST_KIND_IDENTIFIER",
    "AST_KIND_NUMBER_LITERAL",
    "AST_KIND_BINOMIAL_EXPRESSION",
    "AST_KIND_UNARY_EXPRESSION",
    "AST_KIND_FUNCTION_DEFINITION",
    "AST_KIND_FUNCTION_ARGUMENT",
    "AST_KIND_RETURN_STATEMENT",
    "AST_KIND_VARIABLE_ASSIGNMENT",
    "AST_KIND_BOOLEAN_LITERAL",
    "AST_KIND_IF_STATEMENT",
    "AST_KIND_IF_CONDITION",
    "AST_KIND_ELSE_STATEMENT",
};

static char* NodeType[] = {
    "AST_NODE_TYPE_BLANK",
    "AST_NODE_TYPE_NUMBER",
    "AST_NODE_TYPE_BOOLEAN"
};

static char* BinomialExpressionType[] = {
    "AST_BINOMIAL_EXPRESSION_TYPE_BLANK",
    "AST_BINOMIAL_EXPRESSION_TYPE_OR",
    "AST_BINOMIAL_EXPRESSION_TYPE_AND",
    "AST_BINOMIAL_EXPRESSION_TYPE_EQUAL",
    "AST_BINOMIAL_EXPRESSION_TYPE_GREATER",
    "AST_BINOMIAL_EXPRESSION_TYPE_GREATER_OR_EQUAL",
    "AST_BINOMIAL_EXPRESSION_TYPE_LESS",
    "AST_BINOMIAL_EXPRESSION_TYPE_LESS_OR_EQUAL",
    "AST_BINOMIAL_EXPRESSION_TYPE_NOT_EQUAL",
    "AST_BINOMIAL_EXPRESSION_TYPE_SUM",
    "AST_BINOMIAL_EXPRESSION_TYPE_SUBTRACTION",
    "AST_BINOMIAL_EXPRESSION_TYPE_MULTIPLICATION",
    "AST_BINOMIAL_EXPRESSION_TYPE_DIVISION",
    "AST_BINOMIAL_EXPRESSION_TYPE_POWER",
};

static char* UnaryExpressionType[] = {
    "AST_UNARY_EXPRESSION_TYPE_BLANK",
    "AST_UNARY_EXPRESSION_TYPE_NEGATIVE",
    "AST_UNARY_EXPRESSION_TYPE_NOR",
    "AST_UNARY_EXPRESSION_TYPE_BITWISE_NOR",
};